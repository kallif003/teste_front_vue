<template>
  <Container type="paginationContainer">
    <button
      ...
      id="less"
    >
      <v-icon icon="mdi-chevron-left" :size="45" class="text-v_orange" />
    </button>

    <Button
      btnType="paginationButton"
    ...
    <Button
      btnType="paginationButton"
      class="ml-3 bg-[#f8f8f8] text-black"
      @click="plus"
      :disabled="maxPage === 0 ? true : false"
    >
      <v-icon icon="mdi-dots-horizontal" :size="20" />
    ...
    <button @click="next" :disabled="maxPage === 0 ? true : false" id="plus">
      <v-icon icon="mdi-chevron-right" :size="45" class="text-v_orange" />
    </button>
  </Container>
</template>
...
import { computed, ref, PropType, watchEffect, onMounted } from "vue";
